...
CREATE TABLE IF NOT EXISTS league (
    ...
    sticker_enabled BOOLEAN NOT NULL DEFAULT FALSE,      -- "Die Klebrigsten" (Sticker-Album) für diese Liga aktiv? Default AUS (Migration: migrate_stickers.sql)
    FOREIGN KEY (division_id) REFERENCES division(id)
) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- laenger als 2 Minuten her ist — siehe Guard::authorize())
...
-- Tabelle: sticker (eingefrorenes Album einer Saison; POST /sticker/album/sync ergänzt nur, löscht nie)
...
-- Tabelle: sticker_pack (vergebene Packs; source_key macht die Vergabe idempotent)
...
-- Tabelle: sticker_pull (gezogene Karten, beim Öffnen serverseitig gewürfelt; Sammlung = Summe der Züge)
...
(UUID(), 'transfermarkt_geschlagen', 'Schlag den Markt',           'Schlage die beste freie Elf in einer Saison mindestens 3 Mal',                                   'competition',  NULL, NULL, NULL);
