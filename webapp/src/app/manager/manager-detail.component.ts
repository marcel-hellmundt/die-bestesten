...
export class ManagerDetailComponent {
  ...
  chartData = computed(() => {
    ...
    const linePoints = teams
      .map((t, i) => {
        if (t.season_placement == null || t.season_team_count == null || t.season_team_count <= 1)
          return null;
        ...
        return {
          x: toX(i),
          y,
          placement: t.season_placement,
          teamCount: t.season_team_count,
          color: t.color_secondary ?? t.color,
        };
      })
  ...
  onLogoError(teamId: string) {
    this.logoErrors.add(teamId);
  }
  onPlayerError(playerId: string) {
    this.playerErrors.add(playerId);
  }
