...
class NotificationController extends _BaseController
{
    ...
    protected function get(): mixed
    {
        ...
        if ($this->id === 'unread_count') {
            return [
                'count'           => $this->db->getUnreadCount($GLOBALS['auth_manager_id']),
                'incoming_offers' => $this->db->getIncomingPlayerOfferCount($GLOBALS['auth_manager_id']),
            ];
        }
    ...
    protected function patch(): mixed
    {
        ...
        if ($this->id === 'preferences') {
            ...
            $allowed   = ['matchday_completed', 'achievement_earned', 'h2h_draw', 'direct_offer'];
            if (!$eventType || !in_array($eventType, $allowed) || $enabled === null) {
                http_response_code(422);
                return ['message' => 'event_type (matchday_completed|achievement_earned|h2h_draw|direct_offer) und enabled (bool) erforderlich'];
            }
