            ...
            new Route('player_in_team', 'PlayerInTeam', [
                ...
                'endpoints' => [
                    [
                        'method' => 'GET',
                        'path' => '/player_in_team',
                        'description' => 'Alle aktiven Spieler eines Teams (to_matchday_id IS NULL) mit Position, Preis, Saison-Punkten, aktuellem Club, is_drafted (bool — Spieler per /league/:id/draft_assign zugelost statt gekauft/ersteigert, erkannt über transaction.reason=\'Draft-Zuweisung: {displayname}\' bei diesem Team, unabhängig vom Matchday — gleiche Erkennung auch im former[]-Kader); ?include_former=1 → {current, former, drafted_squad} — former[] enthält nur "echte" Abgänge (Spieler, die eine Weile im Kader waren); zugeloste Spieler, deren Zulosungs-Stint noch am Spieltag 1 selbst (from_matchday_id === to_matchday_id, beides Spieltag 1 der Division/Saison) wieder verkauft wurde, stehen stattdessen in drafted_squad[] (gleiche Felder wie former[], inkl. is_drafted=true) statt in former[] — hat derselbe Spieler daneben noch einen späteren, separaten Kauf+Verkauf beim selben Team, bleibt er zusätzlich in former[] stehen (kann also in beiden Arrays vorkommen); drafted_squad[] enthält zusätzlich alle aktuell noch im Kader stehenden zugelosten Spieler (in_squad=true, gleiche Felder wie current[]) — die am Spieltag 1 wieder verkauften Zulosungen aus former[]-Herkunft tragen dort in_squad=false; drafted_squad[] ist nach Position/Punkte/Marktwert sortiert (wie current[]/former[]), nicht nach in_squad-Status gruppiert; ?player_id → aktuelles Team oder null; ?player_id + ?season_id → Teamhistorie des Spielers in dieser Saison [{team_id,team_name,color,manager_name,alias,from_matchday_number,to_matchday_number,price_paid,is_drafted}] — price_paid = Kaufpreis (aus transaction ermittelt über team_id+from_matchday_id+Reason-Text; null falls keine passende Transaktion gefunden, z.B. bei sehr alten/manuell angelegten Daten); is_drafted = ob genau dieser Team-Stint per /league/:id/draft_assign zugelost statt gekauft/ersteigert wurde (reason=\'Draft-Zuweisung: {displayname}\')',
