            ...
            new Route('noten', 'Noten', [
                ...
                'endpoints' => [
                    [
                        'method' => 'GET',
                        'path' => '/noten',
                        'description' => 'Noten + Punkte aller Spieler der 1. Bundesliga (level=1, country=DE — fest, unabhängig von einer evtl. konfigurierten Liga-Division) für einen Spieltag der aktiven Saison → {season_id, matchdays:[{id,number,start_date,kickoff_date}], matchday:{id,number,start_date,kickoff_date}|null, clubs:[{id,name,short_name,logo_uploaded,players:[{id,displayname,position,grade,points,participation}]}]}; bewusst kein Foto-Feld — Spielerbilder werden auf dieser Guest-Seite nie ausgespielt; matchdays enthält nur bereits angepfiffene Spieltage (kickoff_date <= now); ohne matchday_id wird automatisch der letzte angepfiffene Spieltag gewählt; players nur mit participation starting/substitute, sortiert nach participation (starting vor substitute), dann Position (TOR/ABW/MIT/STU), dann player_in_season.price absteigend; clubs sortiert nach Tabellenplatz der Vorsaison (club_in_season.position, unplatzierte ans Ende); matchday=null (clubs=[]) falls in der aktiven Saison noch kein Spieltag angepfiffen wurde — Guest',
