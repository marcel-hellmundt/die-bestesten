            ...
            new Route('team', 'Team', [
                ...
                'endpoints' => [
                    [
                        'method' => 'GET',
                        'path' => '/team',
                        'description' => 'Alle Teams einer Saison — gibt [{id,team_name,color,color_secondary,season_id,manager_id,manager_name,alias,squad_valid,total_value,position_counts:{GOALKEEPER,DEFENDER,MIDFIELDER,FORWARD},bought,sold,drafted_points,drafted_active_count,drafted_actual_points}] sortiert nach team_name zurück; squad_valid = Mindestanforderungen erfüllt (GK≥1/DEF≥5/MID≥5/FWD≥3), position_counts = aktuelle Kaderbesetzung je Position; bought/sold = Kauf-/Verkaufszahlen der ganzen Saison über alle player_in_team-Stints (nicht nur aktive) — Zulosungen (POST /league/:id/draft_assign) zählen nicht als bought, und als sold nur dann, wenn der Verkauf nach Spieltag 1 erfolgte statt noch am selben Spieltag wie die Zulosung; drafted_points = Saisonpunkte-Summe ALLER jemals per Zulosung erhaltenen Spieler dieses Teams (unabhängig ob/wann verkauft — gleiche Menge wie GET /player_in_team?include_former=1s drafted_squad[]); drafted_active_count = wie viele davon noch aktiv im Kader stehen; drafted_actual_points = davon tatsächlich vom Team geholte Punkte (nur Spieltage, an denen der jeweilige Spieler laut team_lineup wirklich aufgestellt war, nominated=1) — meist kleiner als drafted_points, da nicht jeder zugeloste Spieler durchgehend im Lineup stand — Auth',
                        'query_params' => ['season_id' => 'UUID der Saison (erforderlich)'],
                    ],
                    [
                        'method' => 'GET',
