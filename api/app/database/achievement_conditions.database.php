...
trait AchievementConditionsTrait
{
    ...
    public function check_season_goals(array $managerIds): array
    {
        ...
        $stmt = $this->con_league->prepare(
            "SELECT t.manager_id, t.team_name, t.season_id AS team_season_id, tr.matchday_id, tr.goals AS val
             FROM team t
             JOIN team_rating tr ON tr.team_id = t.id AND tr.invalid = 0
             WHERE t.manager_id IN ($plh) AND tr.matchday_id IN ($mPlh)"
        );
        ...
        foreach ($achievers as $mgr => $data) {
            ...
            $total = $data['total'];
            $result[$mgr] = [
                'reason'    => "{$total} Tore mit {$data['team_name']} ($label)",
                'earned_at' => $lastKickoff[$sid] ?? date('Y-m-d H:i:s'),
                'level'     => $total >= 90 ? 'gold' : ($total >= 80 ? 'silver' : 'bronze'),
            ];
        }
        return $result;
    }
    ...
    public function check_season_assists(array $managerIds): array
    {
        ...
        $stmt = $this->con_league->prepare(
            "SELECT t.manager_id, t.team_name, t.season_id AS team_season_id, tr.matchday_id, tr.assists AS val
             FROM team t
             JOIN team_rating tr ON tr.team_id = t.id AND tr.invalid = 0
             WHERE t.manager_id IN ($plh) AND tr.matchday_id IN ($mPlh)"
        );
        ...
        foreach ($achievers as $mgr => $data) {
            ...
            $total = $data['total'];
            $result[$mgr] = [
                'reason'    => "{$total} Vorlagen mit {$data['team_name']} ($label)",
                'earned_at' => $lastKickoff[$sid] ?? date('Y-m-d H:i:s'),
                'level'     => $total >= 70 ? 'gold' : ($total >= 65 ? 'silver' : 'bronze'),
            ];
        }
        return $result;
    }
    ...
    public function check_bench(array $managerIds): array
    {
        ...
        // Count bench-SDS per manager per matchday
        $counts = [];
        ...
        foreach ($achievers as $mgr => $data) {
            $label = $this->seasonLabel($data['season_start']);
            $result[$mgr] = [
                'reason'    => "{$data['count']} Bank-SDS mit {$data['team_name']}, Spieltag {$data['md_number']} ($label)",
                'earned_at' => $data['kickoff_date'],
            ];
        }
        return $result;
    ...
    public function check_heart(array $managerIds): array
    {
        ...
        // All squad players (nominated + bench)
        $stmt = $this->con_league->prepare(
        ...
        foreach ($lineups as $row) {
            $key = $row['manager_id'] . '|' . $row['matchday_id'];
            if (!isset($groups[$key]))
                $groups[$key] = ['manager_id' => $row['manager_id'], 'team_name' => $row['team_name'], 'matchday_id' => $row['matchday_id'], 'players' => []];
            $groups[$key]['players'][] = $row['player_id'];
        }

        $posMin = ['GOALKEEPER' => 1, 'DEFENDER' => 5, 'MIDFIELDER' => 5, 'FORWARD' => 3];
        ...
        foreach ($groups as $data) {
            ...
            if (!$valid) continue;
            if (count(array_unique($countries)) !== 1) continue;

            foreach ($posMin as $pos => $min) {
                if ($posCounts[$pos] < $min) { $valid = false; break; }
            }
            if (!$valid) continue;

            $achievers[$mid] = ['team_name' => $data['team_name'], 'matchday_id' => $data['matchday_id'], 'nation' => $countries[0]];
        }
    ...
    public function check_defense(array $managerIds): array
    {
        if (empty($managerIds)) return [];
        ...
        $stmt = $this->con->prepare(
            "SELECT player_id, matchday_id, COALESCE(points, 0) AS points
             FROM player_rating WHERE matchday_id IN ($mPlh) AND player_id IN ($pPlh)"
        );
        $stmt->execute([...$mdIds, ...$playerIds]);
        $ratingMap = [];
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row)
        ...
        foreach ($groups as $data) {
            ...
            foreach ($data['players'] as $pid) {
                $pos    = $positionMap[$pid][$seasonId] ?? null;
                $points = $ratingMap[$pid . '|' . $data['matchday_id']] ?? 0;
                if ($pos === 'DEFENDER') $defPts   += $points;
                else                     $otherPts += $points;
            }

            if ($defPts <= $otherPts || $otherPts <= 0) continue;

            $ratio = $defPts / $otherPts;
            if (!isset($best[$mid]) || $ratio > $best[$mid]['ratio'])
                $best[$mid] = ['ratio' => $ratio, 'team_name' => $data['team_name'], 'matchday_id' => $data['matchday_id'], 'def_pts' => $defPts, 'other_pts' => $otherPts];
        }

        $result = [];
        foreach ($best as $mid => $data) {
            $label  = $this->seasonLabel($seasonStartMap[$data['matchday_id']]);
            $mdNum  = $mdNumberMap[$data['matchday_id']];
            $factor = number_format($data['ratio'], 2, ',', '.');
            $result[$mid] = [
                'reason'    => "{$data['team_name']}: Abwehr {$data['def_pts']} vs. {$data['other_pts']} Pkt. (×{$factor}), Spieltag $mdNum ($label)",
                'earned_at' => $kickoffMap[$data['matchday_id']],
            ];
        }
        return $result;
