...
POST     /player_in_season/preview_csv — multipart: Feld "csv" (;-getrennt: ID;Vorname;Nachname;Kurzname;Angezeigter Name;Verein;Position;Marktwert;Punkte;Notendurchschnitt) → {status,season_id,rows[{kicker_id,csv_*,matched_player_id,matched_displayname,matched_club_id,club_logo_uploaded,already_in_season,importable}]}; matcht player über kicker_id (Spalte ID → int nach "pl-k"), club über Namen (exakt, sonst Fuzzy-Fallback bei eindeutigem Treffer); schreibt nichts — Admin
