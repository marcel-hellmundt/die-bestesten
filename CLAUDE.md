...
GET      /player_in_team             — ?team_id (erforderlich) → aktive Spieler mit position, price, points, current_club_id, club_logo_uploaded, is_drafted (bool — per /league/:id/draft_assign zugelost statt gekauft/ersteigert, erkannt über transaction.reason='Draft-Zuweisung: {displayname}' bei diesem Team; auch im former[]-Kader gesetzt); ?include_former=1 → {current, former}; ?player_id → aktuelles Team oder null; ?player_id + ?season_id → Teamhistorie [{team_id, team_name, color, manager_name, alias, from_matchday_number, to_matchday_number, price_paid, is_drafted}] — price_paid = Kaufpreis, ermittelt aus transaction (team_id+from_matchday_id+Reason-Text), null falls keine passende Transaktion; is_drafted = ob dieser Team-Stint per Zulosung (draft_assign) statt Kauf/Gebot zustande kam — Auth
...
PATCH    /offer/:id                  — Body:{team_id, offer_value} — Gebotswert eines pending-Gebots ändern; setzt offer.updated_at auf jetzt (NULL solange nie bearbeitet); 422 wenn < Marktwert oder Budget überschritten — Auth
...
PATCH    /player/:id           — beliebige Kombination aus {first_name, last_name, displayname, country_id, birth_city, date_of_birth, height_cm, weight_kg} (mind. eines erforderlich) — korrigiert Stammdaten eines bestehenden Spielers; erstellt keine neuen Spieler (dafür weiterhin nur CSV-Import bzw. POST /player/create); 400 wenn keine Felder oder displayname leer, 409 bei displayname-Duplikat, 404 wenn nicht gefunden, 422 wenn height_cm/weight_kg <= 0 — Maintainer+
...
GET      /team_rating          — ?season_id → { matchday, ratings[], sds_player, max_matchday_number } letzter gestarteter Spieltag; bei nicht-abgeschlossenem Spieltag: Live-Punkte aus player_rating × team_lineup (fine = 0); jedes ratings[]-Element enthält zusätzlich status_counts:{starting,substitute,not_used,waiting} — Anzahl nominierter Spieler je Einsatz-Status für diesen Spieltag (waiting = noch kein player_rating vorhanden, analog zur Lineup-Statusanzeige in /team_lineup) — Auth
GET      /team_rating/season   — ?season_id → aggregierte Saisontabelle aller Teams (inkl. manager_id je Team), sortiert nach Punkten; fine-Felder sowie luck.lucky/luck.unlucky (Glückspilze/Pechvögel) sind 0 bzw. leer, wenn league.fine_ruleset='none' (Goldene Bürste/Hölzerne Bank/Spieltagssiege bleiben davon unberührt) — Auth
...
GET      /h2h/:id            → Match-Detail {match,matchday,home_team,away_team,home_rating,away_rating,home_lineup[],home_bench[],away_lineup[],away_bench[],odds,predictions,head_to_head[]} mit Spieler-Einzelpunkten (Lineup-Einträge enthalten zusätzlich price, season_points); odds:{home,draw,away} = deterministische Pseudo-Quote (Dezimalquote oder null) rein aus Marktwert+Saisonpunkten der jeweils aufgestellten Spieler (nicht Bank) beider Teams — keine echten Einsätze, nur Orientierung "wer ist auf dem Papier stärker"; predictions vor Anpfiff der Matchday {locked:false,kickoff_date,my_pick:'home'|'draw'|'away'|null,is_current_matchday,is_own_match} (Tipps anderer Manager geheim; is_current_matchday=false bei einer bereits geplanten zukünftigen Matchday, is_own_match=true wenn der eingeloggte Manager eines der beiden Teams führt — in beiden Fällen POST /h2h_prediction 403, Tipp-Karte im Frontend komplett unsichtbar statt nur Buttons zu deaktivieren; is_own_match verhindert Manipulation der Quote über die eigene Aufstellung), danach {locked:true,kickoff_date,entries:[{manager_id,manager_name,alias,pick,odds}]} (odds = zum Zeitpunkt der Tippabgabe für den gewählten Pick angezeigte Pseudo-Quote, siehe POST /h2h_prediction) — Auth; ?preview=1 (nur Admin) liefert zusätzlich {preview:true,preview_entries:[...]} testweise schon vor Anpfiff, ohne locked/my_pick zu ändern; head_to_head[] = alle abgeschlossenen H2H-Matches zwischen genau den beiden Managern dieses Matches (nicht Teams — manager_id ist ligenübergreifend dieselbe ID), liga- und saisonübergreifend über alle Ligen der Instanz hinweg ermittelt, absteigend nach kickoff_date, ohne das aktuell betrachtete Match selbst: [{id,league_id,league_name,season_id,matchday_number,kickoff_date,phase,leg,home_team_id,home_team_name,home_color,home_manager_id,away_team_id,away_team_name,away_color,away_manager_id,home_goals,away_goals}]
...
POST     /h2h_prediction    {match_id, pick:'home'|'draw'|'away', odds?} — Tipp setzen/ändern (Upsert, beliebig oft bis Anpfiff); odds speichert die im Frontend zum Zeitpunkt der Tippabgabe für den gewählten Pick angezeigte Pseudo-Quote unverändert als Snapshot; 404 wenn Match nicht gefunden, 403 wenn Anpfiff der zugehörigen Matchday bereits erfolgt ist, die Matchday nicht die aktuelle ist (kleinste noch nicht abgeschlossene number in Saison+Division) oder der Manager eines der beiden beteiligten Teams selbst führt (Quote sonst über die eigene Aufstellung manipulierbar) — Auth
...
**maintainer_contribution**: id PK, manager_id FK, player_rating_id (cross-DB auf global_schema.player_rating, kein FK), contribution_type ENUM(create/participation/stats/note), created_at — UNIQUE(player_rating_id, contribution_type, manager_id) — trackt, welche Manager an einer player_rating-Zeile mitgewirkt haben (create = Zeile per /player_rating/init angelegt, participation = Startelf/Eingewechselt gesetzt, stats = goals/assists/clean_sheet/sds/red_card/yellow_red_card auf einen truthy/non-zero Wert gesetzt, note = Note gesetzt); akkumuliert statt Upsert — jeder Manager, der eine Kategorie einer Zeile jemals bearbeitet hat, bleibt dauerhaft gelistet (INSERT IGNORE), spätere Bearbeiter überschreiben frühere nicht. Zurücksetzen auf null/0/false trackt nie (auch nicht resetRating()) — nur echtes Eintragen von Daten zählt als Contribution
...
**h2h_prediction**: id PK, match_id FK → h2h_match (ON DELETE CASCADE), manager_id (cross-DB), pick ENUM(home/draw/away), odds DECIMAL(6,2)? (Snapshot der im Frontend zum Zeitpunkt der Tippabgabe für den gewählten Pick angezeigten Pseudo-Quote), result ENUM(open/won/lost) DEFAULT open (von H2HPredictionTrait::evaluateH2HPredictionResults() beim Spieltagsabschluss gesetzt: won wenn pick == tatsächliches Ergebnis aus H2HTrait::h2hGoals(), sonst lost), created_at — UNIQUE(match_id, manager_id) — Tipp auf ein H2H-Match; änderbar (Upsert) bis Anpfiff der zugehörigen Matchday, danach für alle sichtbar (siehe GET /h2h/:id → predictions)
