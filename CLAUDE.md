...
PATCH    /player_rating/:id    — Maintainer+; 403 wenn Spieltag completed; Body: grade, participation, goals, assists, clean_sheet, sds, red_card, yellow_red_card (points wird immer serverseitig berechnet); erhöht sich goals, werden alle Manager der aktuellen Liga, die den Spieler für diesen Spieltag aufgestellt haben (team_lineup.nominated=1), per Web-Push benachrichtigt (event_type lineup_player_goal, bewusst kein In-App-Eintrag, siehe PushSubscriptionTrait::sendPushNotification())
...
POST     /push_subscription    — {endpoint, keys:{p256dh, auth}} — Web-Push-Abo speichern/aktualisieren (Upsert per UNIQUE(endpoint)); 400 wenn Felder fehlen — Auth
DELETE   /push_subscription    — {endpoint} — Abo entfernen (idempotent); 400 wenn endpoint fehlt — Auth
...
GET      /notification/preferences — {in_app:{matchday_completed,achievement_earned,h2h_draw,scouted_player_update},push:{achievement_earned,scouted_player_update,lineup_player_goal}} (je bool) — zwei getrennt schaltbare Channels pro event_type, nicht jedes event_type unterstützt beide (lineup_player_goal nur push, siehe NotificationTrait::NOTIFICATION_CHANNELS); fehlende DB-Einträge = true (default ON) — Auth
