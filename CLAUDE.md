...
GET      /player_in_season/available_players — ?season_id (optional, default aktiv) → {players[{id,displayname,position,price,season_points,photo_uploaded,club_id,club_name,club_short_name,club_logo_uploaded,season_id,current_team_id,current_team_name,current_team_season_id}]} — Spieler der konfigurierten Liga-Division ohne Fantasy-Team; ?include_all=1 → auch Spieler mit Fantasy-Team, current_team_* dann gesetzt (sonst null)
