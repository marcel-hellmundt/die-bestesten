...
GET      /achievement          — [{id,name,description,icon,type,threshold_bronze,threshold_silver,threshold_gold,earned_at,reason,seen_at,level,earned_count,total_managers}] — earned_at+reason+seen_at+level=null wenn nicht verdient; threshold_*=null bei Achievements ohne Stufen; description enthält '{threshold}' als Platzhalter bei gestuften Achievements; level='bronze'|'silver'|'gold' (Achievements ohne Stufen immer 'gold'); type='season'|'rating'|'lineup'|'transfer'|'data'; sortiert nach earned_count DESC — Auth; ?all=true → [{id,condition_key,name,description,icon,type,threshold_bronze,threshold_silver,threshold_gold,earned_count,total_managers,managers[{id,manager_name,earned_at,level}]}] — Admin
